/**
 * utils/email.js
 * Email notification utility for Petrichor Naturals — sends order confirmations to customers.
 *
 ...
 *   EMAIL_FROM       — Sender email display (e.g., "Petrichor Naturals <noreply@Petrichor.com>")
 *
 * For Gmail:
...
async function sendOrderConfirmationEmail({ customerEmail, customerName, orderData }) {
  const from = process.env.EMAIL_FROM || `"Petrichor Naturals" <${process.env.EMAIL_USER}>`;
...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f5f5f5;">
          ...
          <tr>
            <td style="background: #1a1a1a; padding: 24px 40px; text-align: center;">
              <p style="margin: 0 0 8px; color: rgba(255,255,255,0.8); font-size: 13px;">
                Thank you for choosing Petrichor Naturals
              </p>
  ...
  const mailOptions = {
    from,
    to: customerEmail,
    subject: `Order Confirmation #${orderData.id} — Petrichor Naturals`,
    html: htmlBody,
  };
